...
class MotionVariationalAutoEncoderClass(nn.Module):
    def __init__(
        self,
        name     = 'CVAE',              
        ...
        """
            Initialize
        """
        super(MotionVariationalAutoEncoderClass,self).__init__()
        self.name     = name
        self.x_dim    = x_dim
        self.y_dim    = y_dim
        self.c_dim    = c_dim
        ...
        self.test()
        # Running Mean Std
        self.running_mean_std = RunningMeanStd(self.x_dim)
    ...
    def init_layers(self):
        ...
        # Decoder part (condition = state)
        h_dim_prev = self.z_dim + self.x_dim
        for h_idx,h_dim in enumerate(self.h_dims[::-1]):
            ...
            self.layers['dec_%02d_actv'%(h_idx)] = \
                self.actv_dec
            h_dim_prev = h_dim
        self.layers['out_lin'] = nn.Linear(h_dim_prev,self.y_dim,bias=True)
        

        # prior part
        h_dim_prev = self.x_dim
        for h_idx,h_dim in enumerate(self.h_dims):
            self.layers['prior_%02d_lin'%(h_idx)]  = \
                nn.Linear(h_dim_prev,h_dim,bias=True)
            self.layers['prior_%02d_actv'%(h_idx)] = \
                self.actv_enc
            h_dim_prev = h_dim
        self.layers['prior_m_lin']  = nn.Linear(h_dim_prev,self.z_dim,bias=True)
        # self.layers['z_var_lin'] = nn.Linear(h_dim_prev,self.z_dim,bias=True)
        ...
        self.motion_vae_parameters = nn.ParameterDict(self.param_dict)
    ...
    def x_to_prior(
        ...
        """
            state and next state to prior mean (fixed prior variance)
        """
        net = x
        for h_idx,_ in enumerate(self.h_dims):
            net = self.layers['prior_%02d_lin'%(h_idx)](net)
            net = self.layers['prior_%02d_actv'%(h_idx)](net)
        prior = self.layers['prior_m_lin'](net)
        return prior
    ...
    def xc_to_y(
        self,
        x             = torch.randn(2,784),
        prev_x        = torch.randn(2,784), # pid prev
        c             = torch.randn(2,10), # phase condition
        STOCHASTICITY = True
        ):
        """
            x and c to y
        ...
        y = self.zc_to_y(z=z_sample,c=x)
        if self.pid is not None:
            self.pid.update(x_trgt=x[...,11:48],x_curr=prev_x[...,11:48],t_curr=0.0,VERBOSE=False)
            self.pid.update(x_trgt=x[...,11:48]+y,x_curr=x[...,11:48],t_curr=0.0083,VERBOSE=False)
            y = self.pid.out()
    ...
    def sample_x(
        ...
        return self.zc_to_y(z=z_sample,c=c),z_sample
    ...
    def test(
        ...
        prior_test   = torch.randn(batch_size,self.x_dim)
        if self.c_dim > 0:
        ...
        y  = self.zc_to_y(z=z_test,c=x_test)
        z_sample = self.xc_to_z_sample(x=x_test,c=c_test)
        y  = self.xc_to_y(x=x_test,prev_x=x_test,c=c_test)
        self.x_to_prior(x=prior_test)
    
    def loss_recon(
        self,
        x               = torch.randn(2,784),
        prev_x          = torch.randn(2,784),
        y_GT            = torch.randn(2,784),
        c               = torch.randn(2,10),
        LOSS_TYPE       = 'L1',
        recon_loss_gain = 1.0,
        STOCHASTICITY   = True
        ...
        """
            Recon loss
        """
        y = self.xc_to_y(x=x,prev_x=prev_x,c=c,STOCHASTICITY=STOCHASTICITY)
        if (LOSS_TYPE == 'L1') or (LOSS_TYPE == 'MAE'):
            errs = torch.mean(torch.abs(y_GT-y),axis=1)
        elif (LOSS_TYPE == 'L2') or (LOSS_TYPE == 'MSE'):
            errs = torch.mean(torch.square(y_GT-y),axis=1)
        elif (LOSS_TYPE == 'L1+L2') or (LOSS_TYPE == 'EN'):
            errs = torch.mean(
                0.5*(torch.abs(y_GT-y)+torch.square(y_GT-y)),axis=1)
        else:
    ...
    def loss_kl(
        ...
        # z_p_mu      = self.x_to_prior(x=x[:,:int(x.shape[1]/2)])
        z_p_mu      = self.x_to_prior(x=x)
        z_p_var     = torch.ones_like(z_var) * 0.22
        z_p_logvar  = torch.log(z_p_var)
        z_logvar    = torch.log(z_var)
        errs        = 0.5*torch.sum(z_var + z_mu**2 - 1.0 - z_logvar,axis=1)*0.1
        # errs        = 0.5*torch.sum(z_p_var + z_p_mu**2 - 1.0 - z_p_logvar,axis=1)*0.1
        errs        +=0.5*torch.sum( - 1.0 - z_logvar + z_p_logvar + (z_var + (z_mu-z_p_mu)**2)/z_p_var)
