...
class PID_ControllerClass(object):
    def __init__(self,
        ...
        # Buffers
        self.cnt      = 0
        self.x_trgt   = torch.zeros(size=(self.dim,))
        self.x_curr   = torch.zeros(size=(self.dim,))
        self.out_val  = torch.zeros(size=(self.dim,))
        self.out_val_prev = torch.zeros(size=(self.dim,))
        self.t_curr   = 0.0
        self.t_prev   = 0.0
        self.err_curr = torch.zeros(size=(self.dim,))
        self.err_intg = torch.zeros(size=(self.dim,))
        self.err_prev = torch.zeros(size=(self.dim,))
        self.p_term   = torch.zeros(size=(self.dim,))
        self.d_term   = torch.zeros(size=(self.dim,))
        self.err_out  = torch.zeros(size=(self.dim,))
    ...
    def update(
        ...
        if x_curr is not None:
            ...
            if self.dt > self.dt_min:
                ...
                self.out_val  = torch.clip(
                    input     = self.p_term + self.i_term + self.d_term,
                    min = torch.tensor(self.out_min),
                    max = torch.tensor(self.out_max))
